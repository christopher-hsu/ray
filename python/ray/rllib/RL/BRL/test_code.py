...
ktd = brl.ktd_Q('loop',0.95, TH=10000)
ktd.learning('egreedy', 0.1, kappa=1, eval_greedy=True)
avg_rew = np.mean(ktd.test_rewards[-10:])
if avg_rew >= 75.0:
	print("KTD-Q eps greedy: Reached an optimal policy... Passed the test!")
else:
	print(avg_rew)
	raise ValueError("It was unable to reach an optimal policy")
...
avg_rew = np.mean(ktd.test_rewards[-10:])
if avg_rew >= 75.0:
	print("KTD-Q active learning: Reached an optimal policy... Passed the test!")
else:
	print(avg_rew)
	raise ValueError("It was unable to reach an optimal policy")