# *************************************************************************************************
# Modified from rl/core.py in keras-rl (https://github.com/matthiasplappert/keras-rl)
...
print("CALLING MODIFIED CORE.PY ...")
class Agent(object):
    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
            ...
            nb_max_episode_steps=None, nb_steps_epoch = None, init_step=None, epoch_file_path=None):
        ...
        episode = 0
        self.step = init_step if init_step else 0
        observation = None
        episode_reward = None
        episode_step = None
        did_abort = False
        ...
        try:
            while self.step < nb_steps:
                ...
                if not(env.env.spec.id == 'CartPole-v0') and self.step > 0 and self.step%nb_steps_epoch==0:
                    cloned_state = env.env.clone_state()
                    test_env = gym.make(env.env.spec.id) # Test with a different environment (HJ)
                    h_test, avg_ep_rew = self.test_epoch(test_env, nb_episodes=10, visualize=False)
                    avg_ep_rewards.append((self.step/nb_steps_epoch, avg_ep_rew ))
                    with open(epoch_file_path, 'w') as f:
                        pickle.dump(avg_ep_rewards, f)
                    env.env.restore_state(cloned_state)
                episode_step += 1
                self.step += 1
