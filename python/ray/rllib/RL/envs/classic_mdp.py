...
def model_assign(env_name):
	if env_name == 'chain': 
		...
		return MiniMaze()
	elif env_name == 'Tsitsiklis':
		return Tsitsiklis()
	elif env_name == 'movingmaze':
		return MovingMaze()
	elif env_name == 'binarytree':
		return BinaryTreeMDP(num_layers=3)
	elif env_name == 'simple':
		return Simple()
	elif env_name == 'simple_d':
		m = Simple()
		m.slip = 0.0
		return m
	elif env_name == 'simple_StoTrans':
		return SimpleStoTrans()
	else:
...
class Simple(Models):
	...
	def optQ(self,discount):
		fname ="optQ/"+self.name+"_"+str(int(discount*100))+"_"+str(int(self.slip_r*10))+"_"+str(int(self.anum))+".npy"
		try:
			Q = np.load(fname)
		except:		
			print("No Stored Data for optQ")
...
class Chain(Models):
	...
	def optQ(self,discount):
		fname = "optQ/chain_"+str(int(discount*100))+"_"+str(int(self.slip*10))+"_"+str(int(self.snum))+".npy"
		try:
			Q = np.load(fname)
		except:		
			print("No Stored Data for optQ")
...
class MovingMaze(Models):
	...
	def change(self):
		self.map = np.asarray(["SOO","OWW", "OOG"], dtype='c')
		self.obstacles = [(1,1),(1,2)]
		self.eff_states = [0,1,2,3,6,7]
	...
	def observe(self,state,action,np_rand):
		...
		if (r_next == self.goal_pos[0]) and (c_next == self.goal_pos[1]): # Reach the exit
			return 10.0, r_next*3 + c_next, True
		else:
			if (r_next,c_next) in self.obstacles: # obstacle tuple list
				return 0.0, state, False
			else: 
				return 0.0, r_next*3 + c_next, False
