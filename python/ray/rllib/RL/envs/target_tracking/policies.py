...
class RandomPolicy():
    def __init__(self):
        self.limit = np.array([[-2.0, -np.pi], [2.0, np.pi]])
...
class SinePolicy():
    def __init__(self, x_interval, a, b, sampling_period):
        self.x_interval = x_interval
        self.a = a # constant factor to x
        self.b = b # y magnitude
        self.sampling_period = sampling_period
        self.limit = np.array([[-1.0, -np.pi], [1.0, np.pi]])
...
class CirclePolicy():
    def __init__(self, sampling_period, maporigin, d_th):
        self.sampling_period = sampling_period
