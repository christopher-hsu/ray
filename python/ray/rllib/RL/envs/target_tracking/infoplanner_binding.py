...
class Configure(object):
    ...
    def setup_integrator_targets(self, n_targets=1, init_pos=None, init_vel=(0.0, 0.0), max_vel=1.0, tau=0.5, q=0.0):
        ...
        [world_model.addTarget(i, IGL.DoubleInt2D(i, init_pos[i], vel, tau, max_vel, q)) for i in range(0, n_targets)]
        return world_model
    ...
    def setup_integrator_belief(self, n_targets=1, init_pos=None, init_vel=(0.0, 0.0), max_vel=1.0, tau=0.5, q=0.0, cov_pos=.25, cov_vel=.1):
        ...
        [belief_model.addTarget(i, IGL.DoubleInt2DBelief(
            IGL.DoubleInt2D(i, init_pos[i], vel, tau, max_vel, q), sigma)) for i in
    ...
    def setup_static_targets(self, n_targets=1, init_pos=None, q=0.0):
        ...
        world_model = IGL.target_model(self.map_nd, self.cmap)
        # Add All targets to belief
        [world_model.addTarget(i, IGL.Static2D(i, init_pos[i], q)) for i in range(0, n_targets)]
        return world_model
    ...
    def setup_static_belief(self, n_targets=1, init_pos=None, q=0.0, cov_pos=0.25):
        ...
        belief_model = IGL.info_target_model(self.map_nd, self.cmap)
        # Add All targets to belief
        [belief_model.addTarget(i, IGL.Static2DBelief(IGL.Static2D(i, init_pos[i], q), cov_pos * np.identity(2))) for i in
         range(0, n_targets)]
        return belief_model
    ...
    def setup_se2_targets(self, n_targets=1, init_odom=None, policy=Policy.zero_policy, tau=0.5, q=0.0):
        ...
        [world_model.addTarget(i, IGL.SE2Target(i, np.array(init_odom[i]), policy=controller, tau=tau,
                                                     q=q))
         for i in
         range(0, n_targets)]
        return world_model
    ...
    #     belief_model = IGL.info_target_model(self.map_nd, self.cmap)
    #     controller = IGL.SE2Policy(policy)
    #     [belief_model.addTarget(i, IGL.SE2Belief(IGL.SE2Target(i, np.array(init_odom[i]), policy=controller, tau=tau,
