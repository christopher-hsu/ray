...
class RosLog(object):
    ...
    def log_single(self, env_i): # TO DO : Integrate Single/Multi
        n = 0 
        env = env_i 
        ...
        t_state = np.concatenate((env.target.state[:2], [np.arctan2(env.target.state[-1], env.target.state[-2])]))
        b_state = np.concatenate((env.belief_target.state[:2], [np.arctan2(env.belief_target.state[-1], env.belief_target.state[-2])]))
        self.targets.append([t_state])
        self.belief_targets.append([b_state])
        self.belief_covs.append(env.belief_target.cov)

    def log(self, env_i):
        if self.num_targets == 1:
            self.log_single(env_i)
        else:
            n = 0 
            env = env_i 
            while(n < self.wrapped_num):
