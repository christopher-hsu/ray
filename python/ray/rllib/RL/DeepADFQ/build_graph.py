...
def build_train(make_obs_ph, q_func, num_actions, optimizer_f, grad_norm_clipping=None, gamma=0.9,
...
def tie_breaking( tf_vec, axis=0):
    samples = tf.random_uniform(tf.shape(tf_vec), dtype=tf.float32)
    eq = tf.equal(tf_vec, tf.reduce_max( tf_vec, axis=axis, keepdims=True))
    return tf.argmax(tf.multiply(tf.cast(eq, tf.float32),samples),axis=axis)
